...
export function LearningApp() {
  ...
  function goTo(nextSection: Section) { setSection(nextSection); setMenuOpen(false); setNotice(""); }
  ...
  return <main className="learning-app">
    ...
    <section className="workspace">
        ...
        <div className="dashboard-grid">
          <article className="study-card">
            ...
            <div className="study-content">
              ...
              <div className={`meaning ${revealed ? "shown" : ""}`} aria-live="polite"><b>{word.french}</b><small>{word.example} · {word.exampleFr}</small></div>
      ...
      {!loading && (section === "library" || section === "favorites") && <section className="library-view"><div className="library-head"><div><h2>{section === "favorites" ? "Tes favoris" : "Tout le vocabulaire"}</h2><p>{source.title} · {source.version}</p></div><label className="search"><Search /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Mot, pinyin ou français" /></label></div><div className="filters"><select value={filterLevel} onChange={(event) => setFilterLevel(Number(event.target.value))} aria-label="Filtrer par niveau"><option value={0}>Tous les niveaux</option>{Array.from({ length: 9 }, (_, indexValue) => <option key={indexValue + 1} value={indexValue + 1}>Niveau {indexValue + 1}</option>)}</select><select value={filterTheme} onChange={(event) => setFilterTheme(event.target.value)} aria-label="Filtrer par thème">{themes.map((item) => <option key={item}>{item}</option>)}</select><select value={section === "favorites" ? "favoris" : filterStatus} onChange={(event) => setFilterStatus(event.target.value)} disabled={section === "favorites"} aria-label="Filtrer par statut"><option value="tous">Tous les statuts</option><option value="favoris">Favoris</option><option value="appris">Déjà vus</option><option value="a-revoir">À revoir</option></select></div><div className="word-table">{filtered.filter((item) => section !== "favorites" || snapshot.progress[item.id]?.favorite).map((item) => <article key={item.id}><button className="favorite" onClick={() => void toggleFavorite(item)} aria-label={snapshot.progress[item.id]?.favorite ? "Retirer des favoris" : "Ajouter aux favoris"}>{snapshot.progress[item.id]?.favorite ? "★" : "☆"}</button><b>{item.hanzi}</b><span>{item.pinyin}</span><span>{item.french}</span><small>Niveau {item.level} · {item.theme}</small><button className="listen" onClick={() => speak(item.hanzi)}><Volume2 /> Écouter</button></article>)}{!filtered.filter((item) => section !== "favorites" || snapshot.progress[item.id]?.favorite).length && <div className="empty-state"><Search /><h3>Aucun mot trouvé</h3><p>Modifie les filtres ou ajoute des favoris depuis la bibliothèque.</p></div>}</div></section>}
